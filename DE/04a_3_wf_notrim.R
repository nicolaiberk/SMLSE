...
df$wordfish_nosw <- read.csv('sims/estim_nosw.csv')[,2]
df$wordfish_notrim <- read.csv('sims/estim_notrim.csv')[,2]

# fix dates, min monthly date, save
df$date <- as.Date(df$date, '%Y-%m-%d')
df$my <- lubridate::floor_date(df$date, "month")
write.csv(df, 'sims/DE_wordfish.csv')
